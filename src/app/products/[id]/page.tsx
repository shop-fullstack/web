...
export default function ProductDetailPage({
  ...
  // Subcategory text for breadcrumb
  const subcategory =
    product.category === "소모품" ? "일회용" : product.category;
        ...
        <div className="flex gap-12">
          ...
          <div className="flex flex-1 flex-col">
            ...
            <div className="mt-6 rounded-lg bg-primary-100 p-4">
              <p className="text-sm text-gray-500">
                정상 단가: {product.unit_price.toLocaleString()}원
              </p>
              <p className="mt-1 text-xl font-bold text-primary-700">
                {product.unit} 단가 ({product.unit_amount.toLocaleString()}
                {product.unit === "박스" || product.unit === "봉"
                  ? "개"
                  : product.unit}
                ) {product.price.toLocaleString()}원
              </p>
              <p className="mt-1 text-sm text-gray-500">
                {product.unit_price.toLocaleString()}원/개 / {product.unit} /{" "}
                {product.unit_amount.toLocaleString()}개
              </p>
            </div>
                ...
                <tbody>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="px-4 py-3 text-gray-900">
                      {product.unit_amount.toLocaleString()}
                      {product.unit === "박스" || product.unit === "봉"
                        ? "개"
                        : product.unit}{" "}
                      / {product.unit}
